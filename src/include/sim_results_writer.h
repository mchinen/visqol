 ...
 private:
  ...
  static std::string FormatFVNSIM(const SimilarityResultMsg& sim_res_msg) {
    ...
    for (size_t i = 0; i < sim_res_msg.fvnsim_size(); i++) {
      ss << std::fixed << std::setprecision(6) << "| " << sim_res_msg.fvnsim(i)
         << std::fixed << std::setprecision(3) << " | " << std::setw(9)
         << std::right << sim_res_msg.center_freq_bands(i) << "Hz"
         << " |" << std::endl;
    }
    ss << "--------------------------\n" << std::endl;
    return ss.str();
  ...
  static void WriteDebugJSON(const FilePath& debug_output_path,
    ...
    if (google::protobuf::util::MessageToJsonString(sim_res_msg, &debug_json).ok()) {
